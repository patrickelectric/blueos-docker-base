...
COPY --chmod=0755 ./scripts/build_gst.sh /build_gst.sh
...
# Install some tools
COPY --chmod=0755 ./scripts/install_viu.sh /install_viu.sh
RUN ./install_viu.sh && rm /install_viu.sh

COPY --chmod=0755 ./scripts/install_gping.sh /install_gping.sh
RUN ./install_gping.sh && rm /install_gping.sh

COPY --chmod=0755 ./scripts/install_simple_http_server.sh /install_simple_http_server.sh
RUN ./install_simple_http_server.sh && rm /install_simple_http_server.sh

# Install Pre-built GStreamer
COPY --chmod=0755 --from=gstreamer /artifacts/. /.

# Update links for the installed libraries and check if GStreamer is setup correctly
COPY --chmod=0755 ./scripts/inspect_gst_plugins.sh /inspect_gst_plugins.sh
RUN ldconfig \
